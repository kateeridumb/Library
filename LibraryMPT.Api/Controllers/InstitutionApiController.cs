...
public sealed class InstitutionApiController : ControllerBase
{
    ...
    public InstitutionApiController(LibraryContext context, ILogger<InstitutionApiController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<ActionResult<InstitutionIndexResponse>> Index()
    {
        ...
        {
            return Ok(new InstitutionIndexResponse
            {
                HasFaculty = false,
        ...
        var faculty = await _context.Faculty
            .FromSqlRaw("SELECT * FROM Faculty WHERE FacultyID = @id", new SqlParameter("@id", facultyId.Value))
            .AsNoTracking()
            .FirstOrDefaultAsync();
        ...
        try
        {
            activeSubscription = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT TOP 1
                        SubscriptionID, FacultyID, Name, StartDate, EndDate, DurationDays, Status, RequestedByUserID
                    FROM Subscriptions
                ...
                """, new SqlParameter("@facultyId", facultyId.Value))
                .AsNoTracking()
                .FirstOrDefaultAsync();
        }
        catch (SqlException ex)
        {
            ...
            activeSubscription = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT TOP 1
                        SubscriptionID, FacultyID, Name, StartDate, EndDate,
        ...
        try
        {
            totalStudents = await _context.Database.SqlQuery<int>($"""
            ...
            totalDownloads = await _context.Database.SqlQuery<int>($"""
                SELECT COUNT(*) AS Value
                FROM BookLogs bl
                ...
                WHERE u.FacultyID = {facultyId.Value}
                  AND r.RoleName = 'Student'
                  AND bl.ActionType = 'DOWNLOAD'
            """).SingleAsync();

            totalReads = await _context.Database.SqlQuery<int>($"""
                SELECT COUNT(*) AS Value
                FROM BookLogs bl
                JOIN Users u ON u.UserID = bl.UserID
                JOIN Roles r ON r.RoleID = u.RoleID
    ...
    public async Task<ActionResult<InstitutionSubscriptionsResponse>> Subscriptions()
    {
        ...
        try
        {
            templates = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT TOP 3
                        s.SubscriptionID, s.FacultyID, s.Name, s.StartDate, s.EndDate, s.DurationDays, s.Status, s.RequestedByUserID
        ...
        catch (SqlException ex)
        {
            ...
            templates = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT TOP 3
                        s.SubscriptionID, s.FacultyID, s.Name, s.StartDate, s.EndDate,
                        CAST(NULL AS INT) AS DurationDays,
                        CAST(NULL AS NVARCHAR(50)) AS Status,
                        CAST(NULL AS INT) AS RequestedByUserID
                    FROM Subscriptions s
        ...
        try
        {
            mySubscriptions = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT
                        s.SubscriptionID, s.FacultyID, s.Name, s.StartDate, s.EndDate, s.DurationDays, s.Status, s.RequestedByUserID
                    FROM Subscriptions s
    ...
    public async Task<ActionResult<ApiCommandResponse>> SelectSubscription([FromBody] SelectSubscriptionRequest request)
    {
        var userId = User.GetUserId();
        ...
        try
        {
            template = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT SubscriptionID, FacultyID, Name, StartDate, EndDate, DurationDays, Status, RequestedByUserID
                    FROM Subscriptions
        ...
        catch (SqlException ex)
        {
            ...
            template = await _context.Subscriptions
                .FromSqlRaw("""
                    SELECT SubscriptionID, FacultyID, Name, StartDate, EndDate,
                        CAST(NULL AS INT) AS DurationDays,
                        CAST(NULL AS NVARCHAR(50)) AS Status,
                        CAST(NULL AS INT) AS RequestedByUserID
                    FROM Subscriptions
        ...
        if (templatesCount > 3)
        {
            var availableTemplates = await _context.Subscriptions
                .FromSqlRaw("SELECT TOP 3 * FROM Subscriptions WHERE FacultyID IS NULL ORDER BY SubscriptionID")
                .AsNoTracking()
        ...
        try
        {
            ...
            """,
                ...
                new SqlParameter("@durationDays", template.DurationDays.Value),
                new SqlParameter("@requestedByUserId", userId));
        }
        catch (SqlException ex)
        {
            ...
            await _context.Database.ExecuteSqlRawAsync("""
                ...
                VALUES (@facultyId, @name, GETDATE(), DATEADD(DAY, @durationDays, GETDATE()))
            """,
                new SqlParameter("@facultyId", facultyId.Value),
                new SqlParameter("@name", template.Name),
                new SqlParameter("@durationDays", days));

            return Ok(new ApiCommandResponse
            {
                Success = true,
                Message = $"Подписка \"{template.Name}\" активирована сразу (старый формат базы данных)."
            });
        }
        ...
        {
            Success = true,
            Message = $"Заявка на подписку \"{template.Name}\" успешно отправлена библиотекарю на рассмотрение. После одобрения подписка будет активирована для вашего учебного заведения."
        });
    }
        ...
        [FromQuery] string? sortDir)
    {
        ...
        if (!facultyId.HasValue)
        {
            return Ok(new InstitutionStudentStatsResponse
            {
                HasFaculty = false,
                ErrorMessage = "У вас не назначено учебное заведение"
            });
        ...
        var orderClause = $"{orderColumn} {normalizedSortDir}";
        ...
        try
        {
            var searchCondition = string.IsNullOrEmpty(trimmedSearch)
                ? "1=1"
                : $"u.Username LIKE '%' + {trimmedSearch} + '%' OR u.FirstName LIKE '%' + {trimmedSearch} + '%' OR CONVERT(nvarchar(max), DecryptByKey(u.LastName)) LIKE '%' + {trimmedSearch} + '%'";

            students = await _context.Database.SqlQuery<StudentStatsDto>($"""
                ...
                SELECT
                    ...
                    COALESCE(CONVERT(nvarchar(max), DecryptByKey(u.LastName)), '') AS LastName,
                    (SELECT COUNT(*) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'DOWNLOAD') AS TotalDownloads,
                    (SELECT COUNT(*) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'READ') AS TotalReads,
                    (SELECT COUNT(DISTINCT BookID) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'READ') AS TotalBooksRead,
        ...
        catch (SqlException ex)
        {
            ...
            var searchCondition = string.IsNullOrEmpty(trimmedSearch)
                ? "1=1"
                : $"u.Username LIKE '%' + {trimmedSearch} + '%' OR u.FirstName LIKE '%' + {trimmedSearch} + '%'";
            ...
            students = await _context.Database.SqlQuery<StudentStatsDto>($"""
                SELECT
                    ...
                    COALESCE(u.Username, '') AS Username,
                    COALESCE(u.FirstName, '') AS FirstName,
                    CAST('' AS nvarchar(max)) AS LastName,
                    (SELECT COUNT(*) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'DOWNLOAD') AS TotalDownloads,
                    (SELECT COUNT(*) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'READ') AS TotalReads,
                    (SELECT COUNT(DISTINCT BookID) FROM BookLogs bl WHERE bl.UserID = u.UserID AND bl.ActionType = 'READ') AS TotalBooksRead,
                    (SELECT MAX(ActionAt) FROM BookLogs bl WHERE bl.UserID = u.UserID) AS LastActivityDate
        ...
        [FromQuery] string? sortDir)
    {
        ...
        if (!facultyId.HasValue)
        {
            return Ok(new InstitutionBookStatsResponse
            {
                HasFaculty = false,
                ErrorMessage = "У вас не назначено учебное заведение"
            });
        ...
        var orderClause = $"{orderColumn} {normalizedSortDir}";
        var trimmedSearch = string.IsNullOrWhiteSpace(search) ? null : search.Trim();
        var searchCondition = string.IsNullOrEmpty(trimmedSearch) ? "1=1" : $"b.Title LIKE '%' + {trimmedSearch} + '%'";

        var bookStats = await _context.Database
            .SqlQuery<BookStatisticsDto>($"""
                SELECT
                    b.BookID,
                    b.Title,
                    (SELECT COUNT(*) FROM BookLogs bl
                     ...
                     WHERE bl.BookID = b.BookID
                       AND u.FacultyID = {facultyId.Value}
                       AND bl.ActionType = 'DOWNLOAD') AS TotalDownloads,
                    (SELECT COUNT(*) FROM BookLogs bl
                     JOIN Users u ON u.UserID = bl.UserID
                     WHERE bl.BookID = b.BookID
                       AND u.FacultyID = {facultyId.Value}
                       AND bl.ActionType = 'READ') AS TotalReads,
                    (SELECT COUNT(DISTINCT u.UserID) FROM BookLogs bl
                     JOIN Users u ON u.UserID = bl.UserID
                     WHERE bl.BookID = b.BookID
                       AND u.FacultyID = {facultyId.Value}
                       AND bl.ActionType = 'DOWNLOAD') AS UniqueDownloaders
                FROM Books b
                WHERE EXISTS (
